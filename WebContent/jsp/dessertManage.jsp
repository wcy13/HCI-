...
<body class="main-bg">
	...
	<div class="main">
		<div class="btn-add-shop">
			<button type="button" class="btn btn-success" data-toggle="modal"
				data-target="#myModal">新增商品</button>
		</div>
				...
				<tr>
					...
					<td>
						<button type="button" class="btn btn-success"
							id='js-modify-<%=item.getDid()%>' onclick="modifyDessert(this)">修改</button>
						<button type="button" class="btn btn-danger"
							id='js-delete-<%=item.getDid()%>' onclick="deleteDessert(this)">删除</button>
					</td>
				</tr>
				<%
					}
				...
				<div class="modal-body">
					<form method="post" class="login-form" id="js-add-shop-form">
						<div class="control-group">
							<label class="control-label" style="font-weight: bold">商品名称：</label>
							<div class="controls">
						...
						<div class="control-group">
							...
							<div class="controls">
								<input id="js-image" type="file" class="form-control"
	...
	<div class="modal fade" id="myModal1" tabindex="-1" role="dialog"
				...
				<div class="modal-body">
					<form method="post" class="login-form" id="js-add-shop-form1">
						<div class="control-group">
							<label class="control-label" style="font-weight: bold">商品名称：</label>
							<div class="controls">
								<input id="js-name1" type="text" class="form-control"
									name="name" class="form-control" required />
						...
						<div class="control-group">
							<label class="control-label">商品图片：</label>
							<div class="controls">
								<input id="js-image1" type="file" class="form-control"
									name="image" class="form-control" required />
							</div>
						</div>
		...
		<script type="text/javascript" charset="utf-8">
			function modifyDessert(obj) {
				var id = obj.getAttribute("id");
				id = id.substr(10);
				$(".js-btn-modify").attr("id",id);
				$('#myModal1').modal();
			}
			function deleteDessert(obj) {
				var id = obj.getAttribute("id");
				id = id.substr(10);
				$.ajax({
					type : "post",
					url : "DessertHouse/deleteDessert",
					async : false,
					data : {
						did : id,
					},
			...
			$(document).ready(function() {
				...
				$("#js-add-shop-form").submit(function() {
					...
					var temp = image.substr(12);
					image = "img/dessert/"+temp ;
					$.ajax({
						type : "post",
						url : "DessertHouse/addDessert",
						async : false,
						data : {
							name : $("#js-name").val(),
							image : image,
				...
				$("#js-add-shop-form1").submit(function() {
					var image = $("#js-image1").val();
					var temp = image.substr(12);
					image = "img/dessert/"+temp ;
					$.ajax({
						type : "post",
						url : "DessertHouse/updateDessert",
						async : false,
						data : {
							name : $("#js-name1").val(),
							image : image,
							did:$(".js-btn-modify").attr("id"),
						},
						success : function(data) {
							alert("修改成功");
							location.reload();
